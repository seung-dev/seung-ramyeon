...
import seung.boot.config.security.types.SCommonOAuth2Provider;
import seung.boot.config.security.types.SIdTokenClaimNames;
...
public class SSignSI implements SSignS {
	...
	public ResponseEntity<SResponseBody> signup_username(
		...
		try {
			...
			while(true) {
				...
				String group_code = request_body.group_code();
				...
				query.add("secret", sBCryptPasswordEncoder.encode(request_body.secret()));
	...
	public ResponseEntity<SResponseBody> signin_username(
		...
		try {
			...
			String s_username = request_body.username();
			...
			while(true) {
				...
				UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(
						...
						, request_body.secret()//credentials
						);
	...
	public ResponseEntity<SResponseBody> signin_success(
		...
		try {
			...
			String principal = request_attribute.principal();
			...
			while(true) {
				...
				switch(SCommonOAuth2Provider.valueOf(provider.toUpperCase())) {
					case GOOGLE:
						query.add("sub", attributes.get_text(SIdTokenClaimNames._S_CLAIM_SUB));
					...
					case FACEBOOK:
						break;
					case GITHUB:
						query.add("sub", attributes.get_text(SIdTokenClaimNames._S_CLAIM_ID));
						query.add("email", attributes.get_text("email", ""));
					...
					case KAKAO:
						...
						break;
					case NAVER:
						SLinkedHashMap naver_response = new SLinkedHashMap(attributes.get_map(SIdTokenClaimNames._S_CLAIM_RESPONSE));
						query.add("sub", naver_response.get_text(SIdTokenClaimNames._S_CLAIM_ID));
			...
			s_access_token = SJWT.generate_token(
					...
					, Collections.singletonMap(SJWT._S_ROLES, user_by_oauth2.get_text("roles"))//claims
					);
